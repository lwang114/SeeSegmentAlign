...
parser.add_argument('--n_concepts', type=int, default=65, help='Number of image concept clusters')
parser.add_argument('--permute', action='store_true', help='Permute the image concepts')
parser.add_argument('--n_folds', type=int, default=1, help='Number of folds for cross-validation')
parser.add_argument('--n_iters', type=int, default=40, help='Number of EM iterations')
parser.add_argument('--date', type=str, default='', help='Date of starting the experiment')
args = parser.parse_args()

if args.dataset == 'mscoco2k':
  nExamples = 2541
  dataDir = '/ws/ifp-04_3/hasegawa/lwang114/spring2020/data/'
...
if len(args.date) > 0:
  expDir = '/ws/ifp-04_3/hasegawa/lwang114/spring2020/dnn_hmm_dnn/exp/%s_%s_%s_%s_momentum%.1f_lr%.5f_width%.3f_alpha0_%.3f_nconcepts%d_%s/' % (args.dataset, args.model_type, args.feat_type, args.audio_feat_type, modelConfigs['momentum'], modelConfigs['learning_rate'], modelConfigs['width'], modelConfigs['alpha_0'], nWords, args.date) 
else:
  # TODO
  expDir = '/ws/ifp-04_3/hasegawa/lwang114/spring2020/dnn_hmm_dnn/exp/%s_%s_%s_momentum%.1f_lr%.5f_width%.3f_alpha0_%.3f_nconcepts%d/' % (args.dataset, args.model_type, args.feat_type, modelConfigs['momentum'], modelConfigs['learning_rate'], modelConfigs['width'], modelConfigs['alpha_0'], nWords) 
...
if not os.path.isdir(expDir):
  ...
  os.mkdir(expDir)
  with open(expDir + 'args.txt', 'w') as f:
    f.write(str(args))
