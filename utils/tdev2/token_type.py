...
class TokenType(Measure):
    ...
    def precision(self):
        ...
        if len(self.disc) == 0:
            ...
            self.token_prec = self.token_hit / float(len(self.disc)) # XXX

        # Types precision/recall
        if len(self.type_seen) == 0:
            self.type_prec = np.nan
        else:
            self.type_prec = len(self.type_hit) / float(len(self.type_seen)) # XXX
    ...
    def recall(self):
        ...
        if self.n_token == 0:
            ...
            self.token_rec = self.token_hit / float(self.n_token) # XXX
        # type recall
        if self.n_type == 0:
            self.type_rec = np.nan
        else:
            self.type_rec = len(self.type_hit) / float(self.n_type) # XXX
