...
class Grouping(Measure):
    ...
    def precision(self):
        if len(self.found_types) == 0:
            ...
            prec = sum(self.found_weights[t] * self.found_gold_counter[t]
                       / float(self.found_counter[t]) for t in self.found_types) # XXX
        return prec
    ...
    def recall(self):
        if len(self.gold_types) == 0:
            ...
            rec = sum(self.gold_weights[t] * self.found_gold_counter[t]
                      / float(self.gold_counter[t]) for t in self.gold_types) # XXX
        return rec
    ...
    def get_weights(pairs):
        ...
        weights = {ngram: counter[ngram]/float(len(seen_token)) for ngram in counter} # XXX
