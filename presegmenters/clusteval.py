  ...
  for i_ex, example_id in enumerate(sorted(gold_boundaries, key=lambda x:int(x.split('_')[-1]))):
    ...
    for gold_start, gold_end, gold_unit in zip(cur_gold_boundaries[:-1], cur_gold_boundaries[1:], cur_gold_units):      
      for pred_start, pred_end, pred_unit in zip(cur_pred_boundaries[:-1], cur_pred_boundaries[1:], cur_pred_units):       
        if abs(pred_end - gold_end) <= tol and abs(pred_start - gold_start): # XXX
...
if __name__ == '__main__':
  ...
  if 2 in tasks:
    ...
    landmark_file = '/ws/ifp-53_2/hasegawa/lwang114/data/mscoco/mscoco2k/feats/mscoco2k_subphone_landmarks_dict.npz' 
    landmark_dict = np.load(landmark_file)
    # gold_file = '{}/{}'.format(args.exp_dir, 'mscoco2k_word_units.wrd') 
    gold_file = '{}/{}'.format(args.exp_dir, 'mscoco2k_phone_units.phn') # '/home/lwang114/spring2019/MultimodalWordDiscovery/utils/tdev2/WDE/share/mscoco2k_phone_units.phn'
    # phone2idx_file = '/ws/ifp-53_2/hasegawa/lwang114/data/mscoco/concept2idx_65class.json' 
    ...
    for model_name in model_names:
      print(model_name)
      # XXX pred_file = '{}/discovered_words_{}.class'.format(args.exp_dir, model_name) # '/home/lwang114/spring2019/MultimodalWordDiscovery/utils/tdev2/WDE/share/discovered_words_{}.class'.format(model_name)     
